...
from src.data.dataset import ChildSpeechDataset, WhisperDataCollator, create_train_val_split
...
class TestFullPipelineIntegration:
    ...
    def temp_workspace(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = {
                ...
                "evaluation": {"primary_metric": "wer", "normalize_predictions": True, "normalize_references": True},
            }
    ...
    def test_model_creation_via_factory(self, temp_workspace):
        ...
        with patch("src.models.whisper_model.WhisperForConditionalGeneration") as mock_model_class, patch(
            "src.models.whisper_model.WhisperProcessor"
    ...
    def test_training_step_mock(self, mock_trainer_class, temp_workspace):
        ...
        with patch("src.models.whisper_model.WhisperForConditionalGeneration") as mock_model_class, patch(
            "src.models.whisper_model.WhisperProcessor"
        ) as mock_processor_class, patch("src.data.dataset.WhisperProcessor") as mock_dataset_processor_class:
    ...
    def test_inference_pipeline(self, temp_workspace):
        ...
        with patch("src.models.whisper_model.WhisperForConditionalGeneration") as mock_model_class, patch(
            "src.models.whisper_model.WhisperProcessor"
        ) as mock_processor_class, patch("src.inference.predictor.WhisperProcessor") as mock_predictor_processor_class:
            ...
            # Create predictor
            predictor = Predictor(model_path=config["model"]["pretrained"], device="cpu")
    ...
    def test_inference_from_manifest(self, temp_workspace):
        ...
        with patch("src.models.whisper_model.WhisperForConditionalGeneration") as mock_model_class, patch(
            ...
            results = predictor.predict_from_manifest(
                manifest_path=str(temp_workspace["manifest_path"]), audio_dirs=config["data"]["audio_dirs"]
            )
    ...
    def test_wer_computation(self, temp_workspace):
        ...
        predictions = ["this is a test", "hello world", "the quick brown fox"]
        references = ["this is a test", "hello word", "the quick brown fox jumps"]
    ...
    def test_end_to_end_pipeline_config_to_predictions(self, temp_workspace):
        ...
        with patch("src.data.dataset.WhisperProcessor") as mock_dataset_processor_class, patch(
            "src.models.whisper_model.WhisperForConditionalGeneration"
        ) as mock_model_class, patch("src.models.whisper_model.WhisperProcessor") as mock_model_processor_class, patch(
            "src.inference.predictor.WhisperProcessor"
        ) as mock_predictor_processor_class:
...
class TestPipelineErrorHandling:
    ...
    def test_invalid_manifest_path(self):
        ...
        with patch("src.data.dataset.WhisperProcessor") as mock_processor_class:
            ...
            with pytest.raises(FileNotFoundError):
                ChildSpeechDataset(
                    manifest_path="nonexistent_manifest.jsonl", audio_dirs=["data/audio_0"], processor=mock_processor
                )

    def test_model_factory_invalid_model_name(self):
        """Test handling of unsupported model name."""
        config = {"model": {"name": "unsupported_model_type", "variant": "base"}}
...
class TestPipelineComponentInteraction:
    ...
    def test_data_collator_with_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("src.data.dataset.WhisperProcessor") as mock_processor_class:
                ...
                dataset = ChildSpeechDataset(
                    manifest_path=str(manifest_path), audio_dirs=[str(audio_dir)], processor=mock_processor
                )
    ...
    def test_metrics_with_predictor_output(self):
        ...
        references = ["hello world", "test sentance"]
