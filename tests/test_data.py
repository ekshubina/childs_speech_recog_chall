...
from src.data.audio_processor import convert_to_mono, load_audio, normalize_audio, resample_audio
from src.data.dataset import ChildSpeechDataset, WhisperDataCollator, create_train_val_split, save_manifest
...
class TestChildSpeechDataset:
    ...
    def test_dataset_initialization_with_manifest(self, sample_manifest, audio_dirs, mock_processor):
        ...
        dataset = ChildSpeechDataset(manifest_path=sample_manifest, audio_dirs=audio_dirs, processor=mock_processor)
    ...
    def test_dataset_initialization_with_samples(self, mock_processor):
        ...
        dataset = ChildSpeechDataset(samples=samples, audio_dirs=[], processor=mock_processor)
    ...
    def test_dataset_manifest_not_found(self, mock_processor):
        ...
        with pytest.raises(FileNotFoundError):
            ChildSpeechDataset(manifest_path="nonexistent_manifest.jsonl", audio_dirs=[], processor=mock_processor)
    ...
    def test_dataset_getitem_missing_audio(self, sample_manifest, mock_processor):
        ...
        dataset = ChildSpeechDataset(manifest_path=sample_manifest, audio_dirs=[], processor=mock_processor)
...
class TestDatasetSplitting:
    ...
    def test_create_train_val_split_basic(self, large_manifest):
        ...
        train_samples, val_samples = create_train_val_split(
            large_manifest, val_ratio=0.1, stratify_by="age_bucket", random_seed=42
        )
    ...
    def test_create_train_val_split_stratification(self, large_manifest):
        ...
        train_samples, val_samples = create_train_val_split(
            large_manifest, val_ratio=0.2, stratify_by="age_bucket", random_seed=42
        )
    ...
    def test_create_train_val_split_custom_ratio(self, large_manifest):
        ...
        train_samples, val_samples = create_train_val_split(large_manifest, val_ratio=0.3, random_seed=42)
    ...
    def test_create_train_val_split_no_stratification(self, large_manifest):
        ...
        train_samples, val_samples = create_train_val_split(large_manifest, val_ratio=0.2, stratify_by=None, random_seed=42)
    ...
    def test_save_manifest(self, tmp_path):
        ...
        samples = [{"utterance_id": "U_001", "text": "hello"}, {"utterance_id": "U_002", "text": "world"}]
...
class TestWhisperDataCollator:
    ...
    def test_collator_single_sample(self, mock_processor):
        ...
        features = [{"input_features": torch.randn(80, 3000), "labels": torch.tensor([1, 2, 3]), "utterance_id": "U_001"}]
