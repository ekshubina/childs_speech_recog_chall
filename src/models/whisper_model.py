...
class WhisperModel(BaseASRModel):
    ...
    def load(self, checkpoint_path: Optional[Union[str, Path]] = None) -> None:
        ...
        try:
            ...
            self.processor = WhisperProcessor.from_pretrained(checkpoint_path, language="english", task="transcribe")
        ...
        **kwargs,
        ...
        for i in range(0, len(audio_paths), batch_size):
            ...
            try:
                ...
                with torch.no_grad():
                    generated_ids = self.model.generate(inputs["input_features"], language=language, task=task, **kwargs)
...
def prepare_model_for_finetuning(
    ...
    if freeze_encoder:
        ...
        logger.info(
            f"Trainable parameters: {trainable_params:,} / {total_params:,} " f"({100 * trainable_params / total_params:.2f}%)"
        )
    else:
