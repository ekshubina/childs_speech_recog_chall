...
def _validate_config(config: Dict[str, Any], config_path: str) -> None:
    ...
    if missing_sections:
        raise ConfigValidationError(f"Missing required configuration sections in {config_path}: {', '.join(missing_sections)}")
...
def _validate_model_config(model_config: Dict[str, Any], config_path: str) -> None:
    ...
    if model_config["name"] not in valid_models:
        raise ConfigValidationError(
            f"Invalid model name '{model_config['name']}' in {config_path}. " f"Must be one of: {', '.join(valid_models)}"
        )
...
def _validate_data_config(data_config: Dict[str, Any], config_path: str) -> None:
    ...
    if not isinstance(data_config["audio_dirs"], list):
        raise ConfigValidationError(f"'data.audio_dirs' must be a list in {config_path}")

    if len(data_config["audio_dirs"]) == 0:
        raise ConfigValidationError(f"'data.audio_dirs' cannot be empty in {config_path}")
...
def _validate_training_config(training_config: Dict[str, Any], config_path: str) -> None:
    ...
    if training_config["batch_size"] <= 0:
        raise ConfigValidationError(f"'training.batch_size' must be positive in {config_path}")

    if training_config["num_epochs"] <= 0:
        raise ConfigValidationError(f"'training.num_epochs' must be positive in {config_path}")


def _check_required_fields(
    config_section: Dict[str, Any], required_fields: List[str], section_name: str, config_path: str
) -> None:
    """
    ...
    if config_section is None:
        raise ConfigValidationError(f"Configuration section '{section_name}' is empty in {config_path}")

    missing_fields = [field for field in required_fields if field not in config_section]

    if missing_fields:
        raise ConfigValidationError(
            f"Missing required fields in '{section_name}' section of {config_path}: " f"{', '.join(missing_fields)}"
        )
