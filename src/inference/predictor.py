...
from src.data.audio_processor import convert_to_mono, load_audio
...
class Predictor:
    ...
    def _load_model(self):
        ...
        try:
            # Load processor
            self.processor = WhisperProcessor.from_pretrained(str(self.model_path), language=self.language, task=self.task)
    ...
    def predict_batch(self, audio_paths: List[Union[str, Path]], show_progress: bool = True) -> List[str]:
        """
        ...
        for i in iterator:
            ...
            try:
                ...
                inputs = self.processor(batch_audio, sampling_rate=16000, return_tensors="pt", padding=True)
                ...
                with torch.no_grad():
                    generated_ids = self.model.generate(
                        inputs["input_features"], language=self.language, task=self.task, max_length=225
                    )
    ...
    def predict_from_manifest(
        ...
        for entry in manifest_data:
            ...
            if audio_path is None:
                raise FileNotFoundError(
                    f"Audio file not found: {audio_filename} " f"(searched in {len(audio_dirs)} directories)"
                )
        ...
        for utterance_id, prediction in zip(utterance_ids, predictions):
            results.append({"utterance_id": utterance_id, "orthographic_text": prediction})
