...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Evaluate trained Whisper model on validation set")
    parser.add_argument("--model-path", type=str, required=True, help="Path to trained model checkpoint")
    parser.add_argument("--val-jsonl", type=str, required=True, help="Path to validation JSONL manifest file")
    parser.add_argument(
        "--audio-dirs",
        type=str,
        nargs="+",
    ...
    parser.add_argument("--output-json", type=str, default=None, help="Path to save evaluation results as JSON")
    ...
    parser.add_argument("--log-file", type=str, default=None, help="Path to log file (default: logs/evaluate_{timestamp}.log)")
...
def compute_metrics_by_age(
    predictions: List[str], references: List[str], age_buckets: List[str], wer_metric: WERMetric
) -> Dict[str, Any]:
    """
    ...
    for age, data in sorted(age_groups.items()):
        if data["predictions"]:  # Skip empty groups
            age_wer = wer_metric.compute(data["predictions"], data["references"], return_details=True)
            age_metrics[age] = {
                "wer": age_wer["wer"],
...
def print_evaluation_results(metrics: Dict[str, Any], logger: logging.Logger):
    ...
    for age in sorted(age_metrics.keys()):
        ...
        logger.info(f"  {age:<15} {age_data['samples']:<10,} " f"{age_data['wer']:<10.4f} {wer_pct:<10.2f}")
...
def main():
    ...
    try:
        ...
        predictor = Predictor(
            model_path=args.model_path, device=args.device, batch_size=args.batch_size, language="english", task="transcribe"
        )
        ...
        results = predictor.predict_from_manifest(
            manifest_path=args.val_jsonl, audio_dirs=args.audio_dirs, output_path=None, show_progress=not args.no_progress
        )
        ...
        metrics = compute_metrics_by_age(predictions, references, age_buckets, wer_metric)
